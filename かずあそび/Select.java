...
public class Select extends JFrame implements Runnable{
	...
	String[] aa = {"  [1]   かぞえる", "  [2]   あわせる", "  [3]   くらべる", "  [4]   おかね　"};
	public void run(){
...
//			JButton bt = new JButton(ra);
			...
			else if(i==3){button.setBackground(Color.CYAN);}
...
//	class RadioAction extends AbstractAction{
		...
		public ClickAction( Select w, int index, InputMap imap, ActionMap amap, CalcTest_pre calc){
			...
			Action num1 = new NumKey("  [1]   かぞえる", this.w, this.calc);
			Action num2 = new NumKey("  [2]   あわせる", this.w, this.calc);
			Action num3 = new NumKey("  [3]   くらべる", this.w, this.calc);
			Action num4 = new NumKey("  [4]   おかね　", this.w, this.calc);
