...
public class Select5 extends JFrame implements Runnable{
	private CalcTest_pre calc;
	private String[] aa = {"","  [1]   1–â (‚à‚ñ)", "  [2]   5–â (‚à‚ñ)", "  [3]   10–â (‚à‚ñ)"};
	public void run(){
		Select5 w = new Select5( "‘I‘ð‰æ–Ê" , this.calc);
	...
	public Select5( String title, CalcTest_pre calc ){
		super( title );
	...
	class ClickAction extends AbstractAction{
		...
		private CalcTest_pre calc;
		private String[] aa = {"  [1]   1–â (‚à‚ñ)", "  [2]   5–â (‚à‚ñ)", "  [3]   10–â (‚à‚ñ)"};
		public ClickAction( Select5 w, int index, InputMap imap, ActionMap amap, CalcTest_pre calc){
			this.w = w;
			this.index = index;
			this.imap = imap;
			this.amap = amap;
			...
			Action num1 = new NumKey("  [1]   1–â (‚à‚ñ)", this.w, this.calc);
			Action num2 = new NumKey("  [2]   5–â (‚à‚ñ)", this.w, this.calc);
			Action num3 = new NumKey("  [3]   10–â (‚à‚ñ)", this.w, this.calc);
	...
	class NumKey extends AbstractAction{
		...
		private Select5 w;
		private CalcTest_pre calc;
		public NumKey(String num, Select5 w, CalcTest_pre calc){
			this.num = num;
			this.w = w;
			this.calc = calc;
